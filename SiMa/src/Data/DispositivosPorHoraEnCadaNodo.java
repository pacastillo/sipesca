 ...
 * @author mgarenas
 */
public final class DispositivosPorHoraEnCadaNodo {
    String         fileName;
    File           f;
    FileWriter     fw;
    BufferedWriter bw;

    public DispositivosPorHoraEnCadaNodo(String lFileName) throws IOException {
        fileName = lFileName;
        createFile();
        openFile();
        writeHead();
    }
    public DispositivosPorHoraEnCadaNodo() throws IOException {
        fileName = "DispositivosPorHoraEnCadaNodo.txt";
        createFile();
        openFile();
        writeHead();
    }
    ...
    public void writeHead() throws IOException {
        bw.write("Nodo\tNombre\tDispositivo\tNºVeces\tAño\tMes\tDia\thora\tFechaCompleta\n");
    }

    public void calcular(Conectar cn) throws SQLException, IOException, ParseException {
        ...
        ResultSet rs = st1.executeQuery("select paso.idNodo, nodo.nombre, idDispositivo, count(idDispositivo), YEAR(FROM_UNIXTIME(tInicio/1000)) as year, MONTH(FROM_UNIXTIME(tInicio/1000)) as month, DAYOFMONTH(FROM_UNIXTIME(tInicio/1000)) as day, HOUR(FROM_UNIXTIME(tInicio/1000)) as hour, FROM_UNIXTIME(tInicio/1000) as fecha from paso, nodo where paso.idNodo=nodo.idNodo group by nodo.IdNodo, idDispositivo, year, month, day, hour;");
        String s;
    ...
    public void calcular(Conectar cn, String fechaInicio, String fechaFin) throws SQLException, IOException, ParseException {
        ...
        // Cuenta el número de dispositivos que han pasado por cada nodo, osea cuántas veces un dispositivo ha pasado por un nodo determinado.
        Statement st1 = cn.crearSt();
        String  query="select paso.idNodo, nodo.nombre, idDispositivo, count(idDispositivo), YEAR(FROM_UNIXTIME(tInicio/1000)) as year, "
                ...
                + "and FROM_UNIXTIME(tInicio/1000) < '"+fechaFin+"' and FROM_UNIXTIME(tInicio/1000) > '"+fechaInicio+"' group by nodo.IdNodo, idDispositivo, year, month, day, hour;";
